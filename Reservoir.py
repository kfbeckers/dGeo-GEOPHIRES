...
class Reservoir:
    ...
    def __init__(self, model:Model):
        ...
        self.gradient1 = self.ParameterDict[self.gradient1.Name] = floatParameter("Gradient 1", value = 0.05, Min=0.0, Max=1000.0, UnitType = Units.TEMP_GRADIENT, PreferredUnits = TemperatureGradientUnit.DEGREESCPERM, CurrentUnits = TemperatureGradientUnit.DEGREESCPERM, Required=True, ErrMessage = "assume default geothermal gradient (50 deg.C/km)", ToolTipText="Geothermal gradient in rock segment")
        self.gradient2 = self.ParameterDict[self.gradient2.Name] = floatParameter("Gradient 2", value = 0.05, Min=0.0, Max=1000.0, UnitType = Units.TEMP_GRADIENT, PreferredUnits = TemperatureGradientUnit.DEGREESCPERM, CurrentUnits = TemperatureGradientUnit.DEGREESCPERM, Required=True, ErrMessage = "assume default geothermal gradient (50 deg.C/km)", ToolTipText="Geothermal gradient in rock segment")
        self.gradient3 = self.ParameterDict[self.gradient3.Name] = floatParameter("Gradient 3", value = 0.05, Min=0.0, Max=1000.0, UnitType = Units.TEMP_GRADIENT, PreferredUnits = TemperatureGradientUnit.DEGREESCPERM, CurrentUnits = TemperatureGradientUnit.DEGREESCPERM, Required=True, ErrMessage = "assume default geothermal gradient (50 deg.C/km)", ToolTipText="Geothermal gradient in rock segment")
        self.gradient4 = self.ParameterDict[self.gradient4.Name] = floatParameter("Gradient 4", value = 0.05, Min=0.0, Max=1000.0, UnitType = Units.TEMP_GRADIENT, PreferredUnits = TemperatureGradientUnit.DEGREESCPERM, CurrentUnits = TemperatureGradientUnit.DEGREESCPERM, Required=True, ErrMessage = "assume default geothermal gradient (50 deg.C/km)", ToolTipText="Geothermal gradient in rock segment")
        self.layerthickness1 = self.ParameterDict[self.layerthickness1.Name] = floatParameter("Thickness 1", value = 2.0, Min=0.01, Max=100.0, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.KILOMETERS, CurrentUnits = LengthUnit.KILOMETERS, ErrMessage = "assume default layer thickness (2 km)", ToolTipText="Thickness of rock segment")
        self.layerthickness2 = self.ParameterDict[self.layerthickness2.Name] = floatParameter("Thickness 2", value = 2.0, Min=0.01, Max=100.0, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.KILOMETERS, CurrentUnits = LengthUnit.KILOMETERS, ErrMessage = "assume default layer thickness (2 km)", ToolTipText="Thickness of rock segment")
        self.layerthickness3 = self.ParameterDict[self.layerthickness3.Name] = floatParameter("Thickness 3", value = 2.0, Min=0.01, Max=100.0, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.KILOMETERS, CurrentUnits = LengthUnit.KILOMETERS, ErrMessage = "assume default layer thickness (2 km)", ToolTipText="Thickness of rock segment")
        ...
        self.fracnumb = self.ParameterDict[self.fracnumb.Name] = intParameter("Number of Fractures", value = 10, AllowableRange=list(range(1,101,1)), UnitType = Units.NONE, ErrMessage = "assume default number of fractures (10)", ToolTipText="Number of identical parallel fractures in EGS fracture-based reservoir model.")
        ...
        self.averagegradient = self.OutputParameterDict[self.averagegradient.Name] = floatParameter("averagegradient", value = 0.0, UnitType = Units.TEMP_GRADIENT, PreferredUnits = TemperatureGradientUnit.DEGREESCPERKM, CurrentUnits = TemperatureGradientUnit.DEGREESCPERM)
        ...
        self.InitialReservoirHeatContent = self.OutputParameterDict[self.InitialReservoirHeatContent.Name] = OutputParameter(Name = "Initial Reservoir Heat Content", value=-999.9, UnitType = Units.ENERGY, PreferredUnits = EnergyUnit.PJ, CurrentUnits = EnergyUnit.PJ)
        ...
        self.gradient = self.OutputParameterDict[self.gradient.Name] = OutputParameter(Name = "Geothermal Gradients", value=[0,0,0,0],  UnitType = Units.TEMP_GRADIENT, PreferredUnits = TemperatureGradientUnit.DEGREESCPERKM, CurrentUnits = TemperatureGradientUnit.DEGREESCPERM)
        self.layerthickness = self.OutputParameterDict[self.layerthickness.Name] = OutputParameter(Name = "Layer Thicknesses", value=[1000000,1000000,1000000,1000000],  UnitType = Units.LENGTH, PreferredUnits = LengthUnit.KILOMETERS, CurrentUnits = LengthUnit.METERS)
            ...
            for item in self.ParameterDict.items():
                ...
                if key in model.InputParameters:
                    ...
                    if ParameterToModify.Name == "Reservoir Model":
                        ...
                        model.reserv.gradient.value[position] = ParameterToModify.value / 1000.0
                        if ParameterToModify.value < 1e-6: model.reserv.gradient.value[position] = 1e-6    #convert 0 C/m gradients to very small number, avoids divide by zero errors later
                            
                    elif ParameterToModify.Name.startswith("Thickness"):
                        parts = ParameterReadIn.Name.split(' ')
                        position = int(parts[1]) - 1
                        model.reserv.layerthickness.value[position] = ParameterToModify.value * 1000.0                        
...
#user-defined functions
    ...
    def Calculate(self, model:Model) ->None:
        ...
        totaldepth = np.append(np.array([0.]), np.cumsum(self.layerthickness.value))
        ...
        #-------------------------------- 
        self.InitialReservoirHeatContent.value = self.resvolcalc.value*self.rhorock.value*self.cprock.value*(self.Trock.value-model.wellbores.Tinj.value)/1E15   #10^15 J = PJ
