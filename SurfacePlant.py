...
class SurfacePlant:
    def __init__(self, model:Model):
        ...
        self.enduseoption = self.ParameterDict[self.enduseoption.Name] = intParameter("End-Use Option", value = EndUseOptions.ELECTRICITY, AllowableRange=[1,2,31,32,41,42,51,52,6,7,8], UnitType = Units.NONE, ErrMessage="assume default end-use option (1: electricity only)", ToolTipText="Select the end-use application of the geofluid heat (see docs for details)")
        self.pptype = self.ParameterDict[self.pptype.Name] = intParameter("Power Plant Type", value = PowerPlantType.SUB_CRITICAL_ORC, AllowableRange=[1,2,3,4], UnitType = Units.NONE, ErrMessage="assume default power plant type (1: subcritical ORC)", ToolTipText="Specify the type of power plant in case of electricity generation. 1: Subcritical ORC, 2: Supercritical ORC, 3: Single-flash, 4: Double-flash")
        self.pumpeff = self.ParameterDict[self.pumpeff.Name] = floatParameter("Circulation Pump Efficiency", value = 0.75, Min=0.1, Max = 1.0, UnitType = Units.PERCENT, PreferredUnits = PercentUnit.TENTH, CurrentUnits = PercentUnit.TENTH, Required=True, ErrMessage="assume default circulation pump efficiency (0.75)", ToolTipText="Specify the overall efficiency of the injection and production well pumps")
        self.utilfactor = self.ParameterDict[self.utilfactor.Name] = floatParameter("Utilization Factor", value = 0.9, Min=0.1, Max = 1.0, UnitType = Units.PERCENT, PreferredUnits = PercentUnit.TENTH, CurrentUnits = PercentUnit.TENTH, Required=True, ErrMessage="assume default utilization factor (0.9)", ToolTipText="Ratio of the time the plant is running in normal production in a 1-year time period.")
        self.enduseefficiencyfactor = self.ParameterDict[self.enduseefficiencyfactor.Name] = floatParameter("End-Use Efficiency Factor", value = 0.9, Min=0.1, Max = 1.0, UnitType = Units.PERCENT, PreferredUnits = PercentUnit.TENTH, CurrentUnits = PercentUnit.TENTH, ErrMessage="assume default end-use efficiency factor (0.9)", ToolTipText="Constant thermal efficiency of the direct-use application")
        self.chpfraction = self.ParameterDict[self.chpfraction.Name] = floatParameter("CHP Fraction", value = 0.5, Min=0.0001, Max = 0.9999, UnitType = Units.PERCENT, PreferredUnits = PercentUnit.TENTH, CurrentUnits = PercentUnit.TENTH, ErrMessage="assume default CHP fraction (0.5)", ToolTipText="Fraction of produced geofluid flow rate going to direct-use heat application in CHP parallel cycle")
        self.Tchpbottom = self.ParameterDict[self.Tchpbottom.Name] = floatParameter("CHP Bottoming Entering Temperature", value = 150.0, Min=0, Max=400, UnitType = Units.TEMPERATURE, PreferredUnits = TemperatureUnit.CELCIUS, CurrentUnits = TemperatureUnit.CELCIUS, ErrMessage="assume default CHP bottom temperature (150 deg.C)", ToolTipText="Power plant entering geofluid temperature used in CHP bottoming cycle")
        self.Tenv = self.ParameterDict[self.Tenv.Name] = floatParameter("Ambient Temperature", value = 15.0, Min = -50, Max = 50, UnitType = Units.TEMPERATURE, PreferredUnits = TemperatureUnit.CELCIUS, CurrentUnits = TemperatureUnit.CELCIUS, ErrMessage="assume default ambient temperature (15 deg.C)", ToolTipText="Ambient (or dead-state) temperature used for calculating power plant utilization efficiency")
        ...
        self.pipinglength = self.ParameterDict[self.pipinglength.Name] = floatParameter("Surface Piping Length", value = 0.0, Min= 0, Max=100, UnitType = Units.LENGTH, PreferredUnits = LengthUnit.KILOMETERS, CurrentUnits = LengthUnit.KILOMETERS, ErrMessage="assume default piping length (5 km)")
        ...
        self.elecprice = self.ParameterDict[self.elecprice.Name] = floatParameter("Electricity Rate", value = 0.08, Min = 0.0, Max=1.0, UnitType = Units.ENERGYCOST, PreferredUnits = EnergyCostUnit.DOLLARSPERKWH, CurrentUnits = EnergyCostUnit.DOLLARSPERKWH, ErrMessage="assume default electricity rate ($0.07/kWh)", ToolTipText="Price of electricity to calculate pumping costs in direct-use heat only mode or revenue from electricity sales in CHP mode.")
        ...
        #absorption chiller
        self.absorptionchillercop = self.ParameterDict[self.absorptionchillercop.Name] = floatParameter("Absorption Chiller COP", value = 0.7, Min=0.1, Max = 1.5, UnitType = Units.PERCENT, PreferredUnits = PercentUnit.TENTH, CurrentUnits = PercentUnit.TENTH, ErrMessage="assume default absorption chiller COP (0.7)", ToolTipText="Specify the coefficient of performance (COP) of the absorption chiller")
        
        #heat pump
        self.heatpumpcop = self.ParameterDict[self.heatpumpcop.Name] = floatParameter("Heat Pump COP", value = 5, Min=1, Max = 10, UnitType = Units.PERCENT, PreferredUnits = PercentUnit.TENTH, CurrentUnits = PercentUnit.TENTH, ErrMessage="assume default heat pump COP (5)", ToolTipText="Specify the coefficient of performance (COP) of the heat pump")
        
        #district heating
        self.dhdemandoption = self.ParameterDict[self.dhdemandoption.Name] = intParameter("District Heating Demand Option", value = 1, AllowableRange=[1,2,3], UnitType = Units.NONE, ErrMessage="assume default district heating demand option (1: known heat demand profile)", ToolTipText="Select the method to provide the district heating demand to GEOPHIRES")
        self.dhdemandfilename = self.ParameterDict[self.dhdemandfilename.Name] = strParameter("District Heating Demand File Name", value = 'HeatDemand.csv', UnitType = Units.NONE, ErrMessage="assume default district heating demand filename (HeatDemand.csv)", ToolTipText="Provide district heating demand in csv file in MW or MWh per day (if district heating demand option is set to 1)")
        self.dhdemandtimeresolution = self.ParameterDict[self.dhdemandtimeresolution.Name] = intParameter("District Heating Demand Data Time Resolution", value = 1, AllowableRange=[1,2], UnitType = Units.NONE, ErrMessage="assume default district heating data time resolution (1: hourly data)", ToolTipText="Provide time interval for thermal demand data: 1 = hourly (data provided as MW = MWh' 2 = daily (data provided as MWh/day) (if district heating demand option is set to 1)")
        self.dhdemanddatacolumnnumber = self.ParameterDict[self.dhdemanddatacolumnnumber.Name] = intParameter("District Heating Demand Data Column Number", value = 2, AllowableRange=list(range(1,101,1)), UnitType = Units.NONE, ErrMessage="assume default district heating demand data column number (2)", ToolTipText="Select the column number of the hourly or daily data in the district heating demand csv file (if district heating demand option is set to 1)")
        self.dhtemperaturefilename = self.ParameterDict[self.dhtemperaturefilename.Name] = strParameter("Temperature File Name", value = 'Temperature.csv', UnitType = Units.NONE, ErrMessage="assume default temperature filename (Temperature.csv)", ToolTipText="Provide filename of tempeature file with hourly temperature to calculate district heating demand (if district heating demand option is set to 2)")        
        self.dhtemperaturedatacolumnnumber = self.ParameterDict[self.dhtemperaturedatacolumnnumber.Name] = intParameter("Temperature Data Column Number", value = 2, AllowableRange=list(range(1,101,1)), UnitType = Units.NONE, ErrMessage="assume default temperature data column number (2)", ToolTipText="Select the column number of the hourly temperature data in the temperature csv file (if district heating demand option is set to 2)")
        self.dhnumberofhousingunits = self.ParameterDict[self.dhnumberofhousingunits.Name] = floatParameter("Number of Housing Units", value = 100, Min=0, Max = 1000000, UnitType = Units.NONE, ErrMessage="assume default number of housing units (100)", ToolTipText="Specify the number of housing units to calculate district heating demand (if district heating demand option is set to 2)")
        self.dhconstantanchordemand = self.ParameterDict[self.dhconstantanchordemand.Name] = floatParameter("Constant Anchor Demand", value = 0, Min=0, Max = 100, UnitType = Units.ENERGY, PreferredUnits = EnergyUnit.MW, CurrentUnits = EnergyUnit.MW, ErrMessage="assume default constant anchor demand (10 MWth)", ToolTipText="Specify the constant anchor demand to calculate the district heating demand (if district heating demand option is set to 2)")
        self.dhuscensusdivision = self.ParameterDict[self.dhuscensusdivision.Name] = intParameter("US Census Division", value = 1, AllowableRange=[1,2,3,4,5,6,7,8,9], UnitType = Units.NONE, ErrMessage="assume default U.S. census division (1)", ToolTipText="Select the U.S. census division to calculate district heating demand (if district heating demand option is set to 2)")
        ...
        self.HeatkWhExtracted = self.OutputParameterDict[self.HeatkWhExtracted.Name] = OutputParameter(Name = "Annual reservoir heat extracted", value = [], UnitType = Units.ENERGYFREQUENCY, PreferredUnits = EnergyFrequencyUnit.KWhPERYEAR, CurrentUnits = EnergyFrequencyUnit.KWhPERYEAR)
        self.PumpingkWh = self.OutputParameterDict[self.PumpingkWh.Name] = OutputParameter(Name = "annual electricity for pumping", value = [], UnitType = Units.ENERGYFREQUENCY, PreferredUnits = EnergyFrequencyUnit.KWhPERYEAR, CurrentUnits = EnergyFrequencyUnit.KWhPERYEAR)
        ...
        self.TotalkWhProduced = self.OutputParameterDict[self.TotalkWhProduced.Name] = OutputParameter(Name = "Total Electricity Generation", value=[0.0], UnitType = Units.ENERGYFREQUENCY, PreferredUnits = EnergyFrequencyUnit.KWhPERYEAR, CurrentUnits = EnergyFrequencyUnit.KWhPERYEAR)
        self.NetkWhProduced = self.OutputParameterDict[self.NetkWhProduced.Name] = OutputParameter(Name = "Net Electricity Generation", value=[0.0], UnitType = Units.ENERGYFREQUENCY, PreferredUnits = EnergyFrequencyUnit.KWhPERYEAR, CurrentUnits = EnergyFrequencyUnit.KWhPERYEAR)
        self.HeatkWhProduced = self.OutputParameterDict[self.HeatkWhProduced.Name] = OutputParameter(Name = "Annual Heat Produced", value=[0.0], UnitType = Units.ENERGYFREQUENCY, PreferredUnits = EnergyFrequencyUnit.KWhPERYEAR, CurrentUnits = EnergyFrequencyUnit.KWhPERYEAR)
        ...
        self.RemainingReservoirHeatContent = self.OutputParameterDict[self.RemainingReservoirHeatContent.Name] = OutputParameter(Name = "Remaining Reservoir Heat Content", value=[0.0], UnitType = Units.ENERGY, PreferredUnits = EnergyUnit.PJ, CurrentUnits = EnergyUnit.PJ)

        #absorption chiller
        self.CoolingProduced= self.OutputParameterDict[self.CoolingProduced.Name] = OutputParameter(Name = "Cooling Produced", value=[0.0], UnitType = Units.ENERGY, PreferredUnits = EnergyUnit.MW, CurrentUnits = EnergyUnit.MW)
        self.CoolingkWhProduced = self.OutputParameterDict[self.CoolingkWhProduced.Name] = OutputParameter(Name = "Annual Cooling Produced", value=[0.0], UnitType = Units.ENERGYFREQUENCY, PreferredUnits = EnergyFrequencyUnit.KWhPERYEAR, CurrentUnits = EnergyFrequencyUnit.KWhPERYEAR)        
        
        #heat pump
        self.HeatPumpElectricityUsed = self.OutputParameterDict[self.HeatPumpElectricityUsed.Name] = OutputParameter(Name = "Heat Pump Electricity Consumed", value=[0.0], UnitType = Units.ENERGY, PreferredUnits = EnergyUnit.MW, CurrentUnits = EnergyUnit.MW)
        self.HeatPumpElectricitykWhUsed = self.OutputParameterDict[self.HeatPumpElectricitykWhUsed.Name] = OutputParameter(Name = "Annual Heat Pump Electricity Consumption", value=[0.0], UnitType = Units.ENERGYFREQUENCY, PreferredUnits = EnergyFrequencyUnit.KWhPERYEAR, CurrentUnits = EnergyFrequencyUnit.KWhPERYEAR)
        
        #district heating
        self.hourlyheatingdemand = self.OutputParameterDict[self.hourlyheatingdemand.Name] = OutputParameter(Name = "Hourly Heating Demand", value=[0.0], UnitType = Units.ENERGYFREQUENCY, PreferredUnits = EnergyFrequencyUnit.MWhPERHOUR, CurrentUnits = EnergyFrequencyUnit.MWhPERHOUR)
        self.dailyheatingdemand = self.OutputParameterDict[self.dailyheatingdemand.Name] = OutputParameter(Name = "Daily Heating Demand", value=[0.0], UnitType = Units.ENERGYFREQUENCY, PreferredUnits = EnergyFrequencyUnit.MWhPERDAY, CurrentUnits = EnergyFrequencyUnit.MWhPERDAY)
        self.annualheatingdemand = self.OutputParameterDict[self.annualheatingdemand.Name] = OutputParameter(Name = "Annual Heating Demand", value=[0.0], UnitType = Units.ENERGYFREQUENCY, PreferredUnits = EnergyFrequencyUnit.GWhPERYEAR, CurrentUnits = EnergyFrequencyUnit.GWhPERYEAR)
        self.utilfactorarray  = self.OutputParameterDict[self.utilfactorarray.Name] = OutputParameter(Name = "Utiliation Factor Array", value=[0.0], UnitType = Units.NONE)
        self.annualngdemand = self.OutputParameterDict[self.annualngdemand.Name] = OutputParameter(Name = "Annual Peaking Boiler Natural Gas Demand", value=[0.0], UnitType = Units.ENERGYFREQUENCY, PreferredUnits = EnergyFrequencyUnit.MWhPERYEAR, CurrentUnits = EnergyFrequencyUnit.MWhPERYEAR)
        self.maxpeakingboilerdemand = self.OutputParameterDict[self.maxpeakingboilerdemand.Name] = OutputParameter(Name = "Maximum Peaking Boiler Natural Gas Demand", value=[0.0], UnitType = Units.ENERGY, PreferredUnits = EnergyUnit.MW, CurrentUnits = EnergyUnit.MW)
        self.dhgeothermalheating = self.OutputParameterDict[self.dhgeothermalheating.Name] = OutputParameter(Name = "Instantaneous Geothermal Heating Over Lifetime", value=[0.0], UnitType = Units.ENERGY, PreferredUnits = EnergyUnit.MW, CurrentUnits = EnergyUnit.MW)
        self.dhnaturalgasheating = self.OutputParameterDict[self.dhnaturalgasheating.Name] = OutputParameter(Name = "Instantaneous Natural Gas Heating Over Lifetime", value=[0.0], UnitType = Units.ENERGY, PreferredUnits = EnergyUnit.MW, CurrentUnits = EnergyUnit.MW)
        ...
        if len(model.InputParameters) > 0:
            ...
            for item in self.ParameterDict.items():
                ...
                if key in model.InputParameters:
                    ...
                    ParameterToModify.CurrentUnits = ParameterToModify.PreferredUnits    #Before we change the parameter, let's assume that the unit preferences will match - if they don't, the later code will fix this.
                    ...
                    if ParameterToModify.Name == "End-Use Option":
                        ...
                        elif ParameterReadIn.sValue == str(31): ParameterToModify.value = EndUseOptions.COGENERATION_TOPPING_EXTRA_HEAT
                        elif ParameterReadIn.sValue == str(32): ParameterToModify.value = EndUseOptions.COGENERATION_TOPPING_EXTRA_ELECTRICTY
                        elif ParameterReadIn.sValue == str(41): ParameterToModify.value = EndUseOptions.COGENERATION_BOTTOMING_EXTRA_HEAT
                        elif ParameterReadIn.sValue == str(42): ParameterToModify.value = EndUseOptions.COGENERATION_BOTTOMING_EXTRA_ELECTRICTY
                        elif ParameterReadIn.sValue == str(51): ParameterToModify.value = EndUseOptions.COGENERATION_PARALLEL_EXTRA_HEAT
                        elif ParameterReadIn.sValue == str(52): ParameterToModify.value = EndUseOptions.COGENERATION_PARALLEL_EXTRA_ELECTRICTY
                        elif ParameterReadIn.sValue == str(6): ParameterToModify.value = EndUseOptions.ABSORPTION_CHILLER
                        elif ParameterReadIn.sValue == str(7): ParameterToModify.value = EndUseOptions.HEAT_PUMP
                        elif ParameterReadIn.sValue == str(8): ParameterToModify.value = EndUseOptions.DISTRICT_HEATING
                    if ParameterToModify.Name == "Power Plant Type":
                        if ParameterReadIn.sValue == str(1): ParameterToModify.value = PowerPlantType.SUB_CRITICAL_ORC
                        elif ParameterReadIn.sValue == str(2): ParameterToModify.value = PowerPlantType.SUPER_CRITICAL_ORC
                        elif ParameterReadIn.sValue == str(3): ParameterToModify.value = PowerPlantType.SINGLE_FLASH
    ...
    def Calculate(self, model:Model) -> None:
        ...
        #heat pump
            ...
            self.HeatProduced.value = self.HeatExtracted.value*self.heatpumpcop.value/(self.heatpumpcop.value-1)*self.enduseefficiencyfactor.value #[MWth]
        ...
        else: #all the other end-use options have electricity generation
        ...
        #---------------------------------------------
        self.HeatkWhExtracted.value = np.zeros(self.plantlifetime.value) #all end-use options have "heat extracted from reservoir" and pumping kWs
        ...
        if self.enduseoption.value != EndUseOptions.ELECTRICITY: #all those end-use options have a heating component
        ...
        if self.enduseoption.value == EndUseOptions.ABSORPTION_CHILLER: #absorption chiller:
            ...
            for i in range(0,self.plantlifetime.value):
                self.CoolingkWhProduced.value[i] = np.trapz(self.CoolingProduced.value[(0+i*model.economics.timestepsperyear.value):((i+1)*model.economics.timestepsperyear.value)+1],dx = 1./model.economics.timestepsperyear.value*365.*24.)*1000.*self.utilfactor.value
    ...
    def CalculateDHDemand(self, model:Model) -> None:
        ...
        if self.dhdemandoption.value == 1: #user provides district heating demand using csv file
            self.dailyheatingdemand.value = self.read_daily_demand(self.dhdemandfilename.value, self.dhdemanddatacolumnnumber.value, self.dhdemandtimeresolution.value)     #obtain daily heating demand
            if self.dhdemandtimeresolution.value == 1:
                self.hourlyheatingdemand.value =  self.read_csv(self.dhdemandfilename.value, self.dhdemanddatacolumnnumber.value)     # if time interval is 1 hour, also store hourly heating demand
       
        elif self.dhdemandoption.value == 2: #calculate thermal demand from TMY and HDD
            self.dailyheatingdemand.value = self.calculatedhdemand(self.dhnumberofhousingunits.value, self.dhuscensusdivision.value, self.dhconstantanchordemand.value, self.dhtemperaturefilename.value, self.dhtemperaturedatacolumnnumber.value)
    ...
    def calculatedhdemand(self,households, census_division, constant_demand, temp_file_name, temp_data_column):
        ...
        daily_HDD = self.calc_HDD(hourlytemperature) # Heating degree days for each day of the year as calculated in calc_HDD
    ...
    def calc_util_factor(self,heatproduced,annualngdemand,timestepsperyear):
        ...
        for i in range(0,self.plantlifetime.value):
            for j in range(0,365):
                ...
                currenttime = i+j/365
                currentheatoutput = np.interp(currenttime,np.arange(0,self.plantlifetime.value+0.01,1/timestepsperyear),heatproduced)
                ...
                if self.dailyheatingdemand.value[j]/24>currentheatoutput:
                    ...
                    instantaneouspeakingboilerdemand[currentindex] = self.dailyheatingdemand.value[j]/24-currentheatoutput
                else:
            ...
            utilfactorarray[i] = sum(actualgeothermalused[i*365:(i+1)*365])/sum(currentheatoutputstored[i*365:(i+1)*365])
                
            
        utilfactor = sum(actualgeothermalused)/sum(currentheatoutputstored)
        if max(instantaneouspeakingboilerdemand) > 0:
            maxpeakingboilerdemand = max(instantaneouspeakingboilerdemand)/20*24                 # max instantaneous peaking boiler demand in MW, assuming it must meet peak demand day running for 20 hours in that day
        else:
            maxpeakingboilerdemand = 0
        
        
        return [utilfactorarray,utilfactor,annualngdemand,maxpeakingboilerdemand,actualgeothermalused,instantaneouspeakingboilerdemand]
